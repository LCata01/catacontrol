...
export function ComplimentaryDialog({
  ...
  const ppt = Number(tt?.people_per_ticket ?? 1);
  const totalPeople = ppt * qty;
  ...
  const save = async () => {
    ...
    toast.success(`Cortesía guardada · ${totalPeople} persona(s)`);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
      <div className="max-h-[90vh] w-full max-w-lg overflow-auto rounded-2xl border border-border bg-card p-6">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-xl font-bold">CORTESÍA</h3>
          <button onClick={onClose} className="text-2xl text-muted-foreground">×</button>
        </div>

        <label className="mb-1 block text-xs uppercase tracking-widest text-muted-foreground">Tipo de ticket</label>
        <div className="mb-4 grid grid-cols-2 gap-2">
          {types?.map((t: any) => (
            <button key={t.id} onClick={() => setTicketTypeId(t.id)}
              ...
              <div className="text-[10px] text-muted-foreground">{t.people_per_ticket ?? 1} pers.</div>
            </button>
          ))}
        </div>

        <label className="mb-1 block text-xs uppercase tracking-widest text-muted-foreground">Nombre del invitado</label>
        ...
        <label className="mb-1 block text-xs uppercase tracking-widest text-muted-foreground">Notas (opcional)</label>
        <input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Prensa, invitado del artista, sponsor…"
          className="mb-4 w-full rounded-lg border border-border bg-input px-4 py-3 outline-none focus:ring-2 ring-ring" />

        <label className="mb-1 block text-xs uppercase tracking-widest text-muted-foreground">Cantidad</label>
        ...
        {tt && (
          <div className="mt-3 text-center text-sm text-muted-foreground">
            Total personas: <span className="font-bold text-foreground">{totalPeople}</span>
          </div>
        )}
