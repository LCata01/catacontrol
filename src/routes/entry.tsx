...
function EntryPos() {
  ...
  const totalPeople = useMemo(
    () => cart.reduce((s, i) => s + (i.kind === "ticket" ? i.people_per_ticket * i.qty : 0), 0),
    [cart]
  );
  ...
  const add = (kind: "ticket" | "wristband", item: any) => {
    setCart((c) => {
      ...
      return [...c, {
        kind, ref_id: item.id, name: item.name, price: Number(item.price), qty: 1,
        people_per_ticket: Number(item.people_per_ticket ?? 1),
      }];
    });
  };
  ...
  const charge = async () => {
    ...
    const items = cart.map((i) => ({
      ...
      people_count: i.kind === "ticket" ? i.people_per_ticket * i.qty : 0,
    }));
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <div className="grid flex-1 gap-4 p-4 lg:grid-cols-[1fr_420px]">
        <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
              {tickets?.map((t: any) => (
                <button key={t.id} onClick={() => add("ticket", t)}
                  ...
                  <div className="text-xs uppercase tracking-widest text-muted-foreground">Ticket · {t.people_per_ticket ?? 1} pers.</div>
          ...
          <Section title="Pulseras">
            <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
              {wristbands?.map((w: any) => (
                <button key={w.id} onClick={() => add("wristband", w)}
                  className="rounded-xl border border-border bg-card p-4 text-left hover:border-primary">
                  <div className="text-xs uppercase tracking-widest text-muted-foreground">Pulsera</div>
        ...
        <div className="flex flex-col rounded-2xl border border-border bg-card">
          <div className="flex-1 overflow-auto p-4">
            {cart.length === 0 && <p className="text-center text-sm text-muted-foreground">Toque ítems para agregar</p>}
            {cart.map((i) => (
              <div key={i.kind + i.ref_id} className="mb-2 flex items-center gap-2 rounded-lg border border-border p-2">
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {i.kind === "ticket" ? "ticket" : "pulsera"} · {money(i.price)}
                    {i.kind === "ticket" && ` · ${i.people_per_ticket} pers.`}
                  </div>
                </div>
          ...
          <div className="border-t border-border p-4">
            ...
            <div className="mb-2 flex items-baseline justify-between">
              ...
              <span className="text-2xl font-black">{totalPeople}</span>
            </div>
